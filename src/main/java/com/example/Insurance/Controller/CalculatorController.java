...
import com.example.Insurance.DTO.SessionUserDTO; // SessionUser DTO 경로
...
public class CalculatorController {
    ...
    /**
     * 보험료 계산기 페이지 뷰(HTML)를 반환합니다.
     * @SessionAttribute를 사용하여 세션의 "user" 정보를 안전하고 간결하게 가져옵니다.
     * @param model 뷰에 데이터를 전달하기 위한 객체
     * @param user  세션에 "user" 속성이 있으면 SessionUser 객체가, 없으면 null이 주입됩니다.
     * @return "calculator" 뷰 이름
     */
    @GetMapping("/calculator")
    public String showCalculatorPage(Model model,
                                     @SessionAttribute(name = "user", required = false) SessionUserDTO user) {
        // 세션에 로그인한 사용자(user)가 있으면, 모델에 user 객체를 추가합니다.
        if (user != null) {
