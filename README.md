...
사용자의 **보장금액**과 **나이와 성별**으로 맞는 상해보험 상품 목록을 제공하여 추천하는 기능을 제공하는 API입니다.

-----

## 💡 핵심 특징

이 API의 가장 중요한 두 가지 핵심 기능은 다음과 같습니다.

  * **보장금액 기준의 명확한 정렬**

      * 복잡한 추천 점수 외에도, 사용자가 가장 중요하게 생각하는 **보장금액**을 기준으로 추천 결과를 정렬할 수 있습니다.
      * **보장금액 높은 순 (`coverage_desc`)** 또는 \*\*낮은 순 (`coverage_asc`)\*\*으로 상품을 나열하여 사용자가 자신의 우선순위에 맞는 상품을 쉽게 찾도록 돕습니다.

  * **나이와 성별에 따른 맞춤 필터링**

      * 사용자의 **나이와 성별**을 핵심 조건으로 받아, 가입이 불가능한 상품을 사전에 완벽하게 필터링합니다.
      * 상품의 상세 조건(`특이사항`)에 명시된 가입 연령을 자동으로 분석하여 실제 가입 가능한 상품만 추천 목록에 포함시킵니다.

  * **객관적인 추천 점수 제공**

      * 정렬 옵션을 선택하지 않을 경우, 보장금액, 보험료(가성비), 갱신주기(안정성)를 종합적으로 평가한 객관적인 **추천 점수** 순으로 상품을 제시합니다.
